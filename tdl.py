...
class TDL(ChannelModel):
    ...
    def __init__(
        ...
        if max_speed is None:
            ...
            assert max_speed >= min_speed, "min_speed cannot be larger than max_speed"
        ...
        alpha_const = (
            ...
            * PI
            / num_sinusoids
            * tf.range(1.0, self._num_sinusoids + 1, 1.0, dtype=real_dtype)
        )
        self._alpha_const = tf.reshape(
            alpha_const,
            [
                1,  # batch size
                1,  # num rx
    ...
    def mean_powers(self):
        ...
        if self._los:
            mean_powers = tf.concat(
                [self._mean_powers[:1] + self._los_power, self._mean_powers[1:]], axis=0
            )
        else:
    ...
    def __call__(self, batch_size, num_time_steps, sampling_frequency):
        # Time steps
        sample_times = (
            tf.range(num_time_steps, dtype=self._real_dtype) / sampling_frequency
        )
        ...
        theta = config.tf_rng.uniform(
            ...
            -PI / tf.cast(self._num_sinusoids, self._real_dtype),
            PI / tf.cast(self._num_sinusoids, self._real_dtype),
            self._real_dtype,
        )
        ...
        normalization_factor = 1.0 / tf.sqrt(
            tf.cast(self._num_sinusoids, self._real_dtype)
        )
        h = tf.complex(
            normalization_factor, tf.constant(0.0, self._real_dtype)
        ...
        if self._los:
            ...
            arg_spec = (
                doppler * sample_times * tf.cos(self._los_angle_of_arrival) + phi_0
            )
            ...
            h = tf.concat(
                [
                    h_spec * tf.sqrt(self._los_power) + h[:, :, :, :, :, :1, :],
                    h[:, :, :, :, :, 1:, :],
                ],
                axis=5,
            )  # Path dims
        ...
        if self._spatial_corr_mat_sqrt is not None:
            ...
            h = split_dim(
                h, [self._num_rx_ant, self._num_tx_ant], tf.rank(h) - 1
            )  # [..., num_rx_ant, num_tx_ant]
            h = tf.transpose(h, [0, 1, 5, 2, 6, 3, 4])
        else:
            if (self._rx_corr_mat_sqrt is not None) or (
                self._tx_corr_mat_sqrt is not None
            ):
    ...
    def _compute_doppler(self, speed):
        ...
        doppler_shift : float
            Doppler shift [Hz]
        """
        return 2.0 * PI * speed / SPEED_OF_LIGHT * self._carrier_frequency
    ...
    def _load_parameters(self, fname):
        ...
        * powers : List of path powers in dB scale
        ...
        source = os.path.join(os.path.dirname(__file__), fname)
